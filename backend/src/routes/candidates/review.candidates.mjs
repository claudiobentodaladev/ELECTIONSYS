...
router.patch("/:candidate_id", isAdmin, (request, response) => {
    ...
    if (!status_candidate) return response.status(400).json(
        new apiResponse("status_candidate must be filled").error(true)
    )

    if (!["eligible", "ineligible", "blocked"].includes(status_candidate)) return response.status(400).json(
        new apiResponse("status_candidate must be eligible, ineligible and blocked").error(true)
    )

    const notReviewedResponse = new apiResponse("There's no candidate to review").error(true)
                                        ...
                                        [candidate_id], (err, result) => {
                                            ...
                                            mysql.execute(
                                                ...
                                                [status_candidate], (err, result) => {
                                                    ...
                                                    return response.status(200).json(new apiResponse(`reviewed the candadidate to ${status_candidate}`).ok({}))
                                                }
                                            )
                                        }
                                    )
