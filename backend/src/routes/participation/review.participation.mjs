...
router.patch("/:election_id/:participation_id", isAdmin, (request, response) => {
    ...
    mysql.execute("select * from elections where user_id = ? and id = ?", [user.id, election_id], (err, result) => {
        ...
        mysql.execute("SELECT status FROM participation WHERE id = ?", [participation_id], (err, result) => {
            ...
            if (status === "voted") {
                return response.status(400).json({ message: " not reviewed the participation, because status is voted", reviewed: false, status: status, participation_id: participation_id })
            }
